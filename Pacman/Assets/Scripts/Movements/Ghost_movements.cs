...
public class Ghost_movements : Grid_character {
    ...
    public Vector3 caseAdjAleatoire()
    {
        ...
        if (listC.Count == 1)
            return listC[0];
        int i = UnityEngine.Random.Range(0, listC.Count); //Choisis un nombre aléatoire entre 0 inclus et la taille de la liste exclu.
        return listC[i];
    }

    //Modifie le son des fantomes en fonction de leur proximité avec Pacman, plus ils sont près de lui plus leur bruit est fort. Spooky ! 
    public void volumeEnFonctionDeDistance()
    {
        float distAvecPacman = dist(Cell, PacmanPos);
        if (distAvecPacman >= 15f)
            fantome_audio.volume = 0f;
        else
        {
            fantome_audio.volume = 0.8f - (distAvecPacman / 15f);
        }
    }
    ...
    public void ChaseAndScatter(Vector3 cible)
    {
        ...
        //trace une ligne du fantome à la cible, utile pour debug ou verifier le comportement des fantomes.
    ...
    public void retourAuSpawn()
    {
        ...
        updateCell(); //On met à jour les informations retenant la position du fantome.
